...
export const youtube = async (req, res) => {
  ...
  try {
    ...
    // const ytList = await YTList.findOne({ slug: slug })

    const ytList = await YTList.findOne({ slug: slug })

    const createdList = await new YTList({
      slug,

      videos: videos,
    }).save()

    // if (createdList.videos[0].length === 0) return
    const newList = await YTList.findOneAndUpdate(
      slug,
